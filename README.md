...
- **Live data** — firm counts and cloud-adoption rates come from the Eurostat API
  (`sbs_sc_ovw`, `isoc_cicce_use`) via [`src/eurostat_data.py`](src/eurostat_data.py),
  with a committed cache for reproducible offline runs. Re-run `--refresh` and the
  whole model updates when Eurostat publishes new data.
- **Uncertainty, quantified** — a vectorised Monte Carlo
  ([`src/monte_carlo.py`](src/monte_carlo.py)) propagates driver distributions through
  the funnel: the answer is "P10 €6m – P90 €19m", not a single false-precision number.
- **Auditability** — every input lives in [`config/assumptions.yaml`](config/assumptions.yaml)
  with its source in a comment; the logic is versioned, testable, and diff-able.
- **Rigor** — unit tests enforce that the funnel narrows, scenarios stay ordered,
  Monte Carlo results reproduce exactly under a fixed seed, and the bottom-up TAM
  stays within tolerance of the top-down cross-check.
...
- **TAM:** ≈ €38bn/yr total EU-27 SME SaaS spend (bottom-up), within ~1% of the top-down cross-check
- **SAM:** ≈ €0.8bn/yr after ICP, region, and category filters
- **SOM (deterministic):** ≈ €11–12m/yr obtainable by year 5 in the base scenario
- **SOM (Monte Carlo):** P10–P90 band around the median across 10,000 draws — run the
  model for exact bands, plus P(SOM > threshold) for go/no-go framing
- **Biggest swing factor:** category wallet share — the least-evidenced assumption dominates the
  tornado, which is exactly where further desk research would be pointed on a real engagement
